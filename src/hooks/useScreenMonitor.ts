...
import {
  ...
  enhanceLLMContextWithOCR,
} from "../lib/ocrIntegrationService";
...
export const useScreenMonitor = (props: ScreenMonitorProps) => {
  ...
  const [actionHistory, setActionHistory] = useState<
    { timestamp: number; app: string; title: string }[]
  >([]);
  ...
  // 画像の差分を計算する関数
  const calculateImageDifference = useCallback(
    async (img1: string, img2: string): Promise<number> => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(
      ...
      Math.max(2000, config.interval),
    ); // 2秒ごと、または監視間隔以上
    return () => clearInterval(interval);
  }, [isMonitoring, config.interval, recordAppSwitch]);

  // 文脈に基づいたアドバイスを生成
  const generateContextualAdvice = useCallback(
    async (
      screenshot: Blob,
      context: string,
      ...
      const memoryContext = relatedMemories.map(
        (memory, idx) =>
      ...
      const actionContext = actionHistory
        .slice(-3)
        .map(
          (item, idx) =>
  ...
  const generateStructuredAdvice = useCallback(
    async (
      screenshot: Blob,
      context: string,
      history: ScreenChange[],
      ...
      if (!geminiApiKey) {
        return {
          raw: "Gemini API Keyが設定されていません。設定画面からAPI Keyを設定してください。",
        };
      }
      const base64 = await new Promise<string>((resolve) => {
        const reader = new FileReader();
  ...
  const monitorScreen = useCallback(async () => {
    try {
      ...
      if (lastScreenshot) {
        const changePercentage = await calculateImageDifference(
          lastScreenshot,
          currentScreenshot,
        );
        if (changePercentage > config.changeThreshold) {
          ...
          // 高度なOCR処理とLLMコンテキスト拡張
          const ocrEnhancement = await ocrIntegrationService.processImageForLLM(
            blob,
            {
          ...
          // フォールバック用の基本OCR
          const fallbackOcrText = await extractOcrText(blob);
          const ocrText = ocrEnhancement.ocrText || fallbackOcrText;
          ...
          if (ocrText && ocrText.length > 0) {
            context += `\n\n[画面OCR解析結果]`;
            context += `\nテキスト: ${ocrText.slice(0, 500)}`;

            if (ocrEnhancement.structuredContent.metadata) {
              context += `\nコンテンツ: ${ocrEnhancement.structuredContent.metadata}`;
            }

            if (ocrEnhancement.structuredContent.extractedData) {
              context += `\n抽出データ: ${ocrEnhancement.structuredContent.extractedData}`;
            }

            if (ocrEnhancement.contentInsights.keyTopics.length > 0) {
              context += `\nトピック: ${ocrEnhancement.contentInsights.keyTopics.join(", ")}`;
            }
          }
          ...
          setScreenHistory((prev) => [...prev.slice(-9), newChange]); // ���新10件を保持

          // スクリーンショットアクションを記録（構造化情報付き）
          await recordScreenshot(context, {
            changePercentage,
            ocrText: ocrText || undefined,
            ocrAnalysis: ocrEnhancement.contentInsights,
            contentType: ocrEnhancement.structuredContent.metadata,
          });

          // 文脈に基づいたアドバイスを生成
          const advice = await generateContextualAdvice(
            blob,
            context,
            screenHistory,
          );
          ...
          // 重要な画面変化は自動的に記憶として保存（構造化情報付き）
          if (changePercentage > 0.3 && ocrText && ocrText.length > 50) {
            const memoryTags = ["screen-change", "auto-saved"];

            // コンテンツタイプに基づくタグ追加
            if (ocrEnhancement.contentInsights.documentType) {
              memoryTags.push(ocrEnhancement.contentInsights.documentType);
            }

            // キートピックをタグに追加
            memoryTags.push(
              ...ocrEnhancement.contentInsights.keyTopics.slice(0, 3),
            );
            ...
            await saveMemory(
              ...
              {
                ocrAnalysis: ocrEnhancement.contentInsights,
                contentStructure: ocrEnhancement.structuredContent,
                detectedLanguage: ocrEnhancement.ocrText ? "detected" : "none",
              },
            );
          }
          ...
          if (props.onStructuredAdvice) {
            const structured = await generateStructuredAdvice(
              blob,
              context,
              screenHistory,
            );
